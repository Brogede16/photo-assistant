...
const termIds = new Set(taxonomy.terms.map((term) => term.id));

for (const term of taxonomy.terms) {
  if (term.parent) assert.equal(termIds.has(term.parent), true, `Tag ${term.id} har ukendt parent ${term.parent}`);
  for (const id of term.implies || []) assert.equal(termIds.has(id), true, `Tag ${term.id} implies ukendt tag ${id}`);
  for (const id of term.related || []) assert.equal(termIds.has(id), true, `Tag ${term.id} related ukendt tag ${id}`);
}

for (const profile of situations.profiles) {
  for (const id of profile.subjects || []) assert.equal(termIds.has(id), true, `Profil ${profile.id} har ukendt subject ${id}`);
  for (const id of profile.requiredTags || []) assert.equal(termIds.has(id), true, `Profil ${profile.id} kræver ukendt tag ${id}`);
  for (const [facet, ids] of Object.entries(profile.conditions || {})) {
    for (const id of ids || []) assert.equal(termIds.has(id), true, `Profil ${profile.id} har ukendt ${facet}-tag ${id}`);
  }
}
